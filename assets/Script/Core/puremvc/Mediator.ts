...
import { _decorator, Component, Node, CCObject, UITransform, isValid } from 'cc';
...
export class Mediator extends Component {
    ...
    private checkSaveEvent = (): void => {
        // Logger.log(this.getModuleView().name + " checkSaveEvent", this._savedUIEventList)
    ...
    protected onModulePreAddToScene(): void {
        Logger.log(this.getModuleView().name + " onModulePreAddToScene")
    }

    protected onModuleAddedToScene(): void {
        Logger.log(this.getModuleView().name + " onModuleAddedToScene")
    }
    ...
    protected getProxy(type: string): any {
        return Facade.getInstance().getProxy(type);
    }
}
